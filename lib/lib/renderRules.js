...
const renderRules = (Text) => ({
    ...
    list_item: (node, children, parent, styles, inheritedStyles) => {
        ...
        const textStyles = pickTextStyles({
            ...(typeof inheritedStyles === 'object' && inheritedStyles !== null
                ? inheritedStyles
                : {}),
    ...
    code_inline: (node, _children, _parent, styles, inheritedStyles) => {
        ...
        return (<Text key={node.key} style={[
                (_a = inheritedStyles) !== null && _a !== void 0 ? _a : {},
                styles.code_inline,
            ]}>
      {node.content}
    </Text>);
    ...
    link: (node, children, _parent, styles, onLinkPress) => (<react_native_1.Pressable accessibilityRole="link" key={node.key} onPress={() => (0, openUrl_1.default)(node.attributes.href, onLinkPress)}>
      <Text style={styles.link}>{children}</Text>
    </react_native_1.Pressable>),
    blocklink: (node, children, _parent, styles, onLinkPress) => (<react_native_1.Pressable accessibilityRole="link" key={node.key} onPress={() => (0, openUrl_1.default)(node.attributes.href, onLinkPress)} style={styles.blocklink}>
    ...
    image: (node, _children, _parent, styles, allowedImageHandlers, defaultImageHandler) => {
        ...
        const handlers = Array.isArray(allowedImageHandlers)
            ? allowedImageHandlers.filter((value) => typeof value === 'string')
            : [];
        const fallbackHandler = typeof defaultImageHandler === 'string' ? defaultImageHandler : null;
        if (!src) {
            return null;
        }
