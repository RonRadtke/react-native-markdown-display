...
const MarkdownTextInput = react_1.default.forwardRef(function MarkdownTextInput({ onChangeText, onCommand, onSelectionChange, inputComponent: InputComponent, selection, style, toolbarItems = DEFAULT_TOOLBAR_ITEMS, compactMaxHeight, enableShortcuts = true, multiline = true, numberOfLines, resolveCommandPayload, value, ...textInputProps }, ref) {
    ...
    const [openMenuLabel, setOpenMenuLabel] = (0, react_1.useState)(null);
    ...
    return (<react_native_1.View style={styles.container}>
            ...
            if (isToolbarMenuItem(item)) {
                ...
                return (<react_native_1.View key={`menu:${item.label}`} style={styles.toolbarMenuContainer}>
                                    <react_native_1.Pressable accessibilityLabel={(_a = item.accessibilityLabel) !== null && _a !== void 0 ? _a : `${item.label} menu`} accessibilityRole="button" accessibilityState={{ expanded: isMenuOpen }} onPress={() => setOpenMenuLabel((currentLabel) => currentLabel === item.label
                        ? null
                        : item.label)} style={[
                        ...
                        isMenuOpen
                            ...
                            : null,
                                        ...
                                        <react_native_1.Text style={styles.toolbarButtonText}>
                                            {item.label}
                                        </react_native_1.Text>
                            ...
                            var _a;
                            return (<react_native_1.Pressable accessibilityLabel={(_a = menuItem.accessibilityLabel) !== null && _a !== void 0 ? _a : DEFAULT_TOOLBAR_ACCESSIBILITY_LABELS[menuItem.command]} accessibilityRole="button" key={menuItem.command} onPress={() => {
                                    handleCommandPress(menuItem.command);
            ...
            return (<react_native_1.Pressable accessibilityLabel={(_b = item.accessibilityLabel) !== null && _b !== void 0 ? _b : DEFAULT_TOOLBAR_ACCESSIBILITY_LABELS[item.command]} accessibilityRole="button" key={item.command} onPress={() => {
                    handleCommandPress(item.command);
                }} style={styles.toolbarButton}>
                                <react_native_1.Text style={styles.toolbarButtonText}>{item.label}</react_native_1.Text>
                            </react_native_1.Pressable>);
        })}
                </react_native_1.View>
