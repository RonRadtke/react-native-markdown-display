...
const getToolbarButtonKey = (item, index) => isToolbarCommandItem(item)
    ...
    : `action:${index}`;
const renderToolbarLabel = (label) => typeof label === 'string' || typeof label === 'number' ? (<react_native_1.Text style={style_1.default.toolbarButtonText}>{label}</react_native_1.Text>) : (<react_native_1.View style={style_1.default.toolbarButtonContent}>{label}</react_native_1.View>);
...
const MarkdownTextInput = react_1.default.forwardRef(function MarkdownTextInput({ onChangeText, onCommand, onSelectionChange, inputComponent: InputComponent, selection, style, toolbarItems = DEFAULT_TOOLBAR_ITEMS, compactMaxHeight, enableShortcuts = true, multiline = true, numberOfLines, resolveCommandPayload, value, ...textInputProps }, ref) {
    ...
    return (<react_native_1.View style={style_1.default.container}>
                {toolbarItems.length > 0 ? (<react_native_1.View style={style_1.default.toolbar}>
                        {toolbarItems.map((item, index) => {
                if (isToolbarMenuItem(item)) {
                    const isMenuOpen = openMenuIndex === index;
                    return (<react_native_1.View key={`menu:${index}`} style={style_1.default.toolbarMenuContainer}>
                            ...
                            ? null
                            : index)} style={[
                            style_1.default.toolbarButton,
                            isMenuOpen
                                ? style_1.default.toolbarButtonActive
                                : null,
                        ]}>
                                            {renderToolbarLabel(item.label)}
                                        </react_native_1.Pressable>
                                        {isMenuOpen ? (<react_native_1.View style={style_1.default.toolbarMenu}>
                                ...
                                }} style={style_1.default.toolbarMenuButton}>
                ...
                return (<react_native_1.Pressable accessibilityLabel={getToolbarButtonAccessibilityLabel(item)} accessibilityRole="button" key={getToolbarButtonKey(item, index)} onPress={() => {
                    ...
                    }} style={style_1.default.toolbarButton}>
                                    {renderToolbarLabel(item.label)}
                                </react_native_1.Pressable>);
            })}
                    </react_native_1.View>) : null}
